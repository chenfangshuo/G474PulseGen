...
> 本文档已对照 `Netlist Schematic.NET` 校正。**注意仓库不提供 BOM，原理图与 PCB 源文件也未公开**（见 §1.1 第 1 项）——因此阻值（如 20Ω / 10k / 4.7k）既不在网表里，也无法在仓库内核对，下文给出的均为原理图标注值，实装以实物为准。
...
若 R62/R63 焊接，MCU 3.3V 脚会与 U8 的 B 侧输出并联。正常经电平转换输出时应 **不贴 R62/R63**（或仅作调试旁路）。R49–R56 阻值原理图常标 20Ω；**仓库不提供 BOM，无法核对实装值**（见文首说明）。
