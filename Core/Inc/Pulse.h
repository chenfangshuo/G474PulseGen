...
/* ===================== 计数时钟基准 =====================
 * HRTIM 的 fHRTIM 取自 **APB2 定时器时钟** (CK_HRTIM)。G4 的 HRTIM 时钟源
 * 不可选 —— HAL 里没有 RCC_HRTIM1CLK_* 之类的选项, 它跟着 APB2 走。
 * 本工程 APB2 分频为 DIV2, 于是 APB2 定时器时钟 = 2 × PCLK2 = 170MHz,
 * **恰好等于 SYSCLK**; 但这是当前分频配置的结果, 不是普遍规律 ——
 * 若改了 APB2 分频, fHRTIM 就不再等于主频。
 *
 * 本工程全部时间换算 (分频选档、死区、SYNC 脉宽、PRF 猝发、多脉冲补偿)
 * 都从这一个值派生 —— 改时钟树只需改这里。
 *
 * ⚠ 必须等于 .ioc 中的 RCC.APB2TimFreq_Value。
 *   改了时钟树却漏改这里, **编译不会报错**, 只会让所有档位的频率与计数值
 *   系统性偏移 —— 表现为"波形整体偏差一个固定比例", 很难定位。
 *   已有构建期检查拦住它 (cmake/check_regen_invariants.cmake),
