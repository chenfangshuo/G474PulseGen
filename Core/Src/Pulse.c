...
/**
 * @warning NPULSE_INTERVAL_COMP_US 是**实测标定**的经验值, 其有效性绑定当前
 *          优化等级与工具链 —— 它补偿的是**中断响应延迟**, 而中断响应延迟直接受
 *          -O3 与 arm-none-eabi-gcc 版本影响。**改动编译优化等级、或升级工具链后
 *          必须重新标定**, 否则多脉冲间隔会系统性偏移。
 *
 * @note    标定依据 (2026-09, -O3, 示波器余晖触发约 10 次, 量同路相邻脉冲上升沿):
 *            设定 1µs  → 实测 1.06~1.12   => 开销 0.96~1.02
 *            设定 2µs  → 实测 2.06~2.10   => 开销 0.96~1.00
 *            设定 4µs  → 实测 4.06~4.10   => 开销 0.96~1.00
 *            设定 8µs  → 实测 8.06~8.07   => 开销 0.96~0.97
 *            设定 16µs → 实测 16.07~16.09 => 开销 0.97~0.99
 *          五点**下界全部落在 0.96**, 且跨越 MUL32/MUL8 等多个分频档无漂移 ——
 *          证明开销是与分频档无关的恒定时间量, 故用常数偏移补偿是正确模型。
 *          取 0.97: 下界(0.96)最可复现, 中点估计为 0.979, 而 ISR 延迟分布通常
 *          右偏, 中位数低于中点; 且 0.96~0.98 之间仅差 10~20ns, 小于测量展宽。
 *          (历史: 曾从 2.0µs 调整为 0.9µs, 0.9 系统性偏小约 70ns。)
 *
 * @note    **改后复测 (同条件, 已收敛)**: 1.005~1.045 / 1.98~2.00 / 3.98~4.02 /
 *          7.99~8.01 / 15.98~16.01。2/4/8/16µs 各点中心残差均 ≤10ns, 远小于
 *          20~40ns 的抖动展宽, 已无可检测的系统性偏差 —— 剩余误差全部来自中断
 *          响应抖动, 非补偿常量所能改善, 故标定到此为止, 不要再调这个值。
 *          1µs 点中心 1.025 = 钳位值 0.05 + 开销 0.975, 与模型吻合。
 *
 * @warning **短间隔的精度上限**: 当 设定间隔 < (COMP + MIN) ≈ 1.02µs 时,
 *          interval_comp 会被 NPULSE_INTERVAL_MIN_US 钳到 0.05, 实际间隔不再
 *          跟随设定值而停在 ~1.02µs 附近。这是既有的设计边界, 不是缺陷。
 *
 * @note    重新标定方法: 多脉冲模式(脉冲数≥5, 关闭 PRF 猝发), 用示波器量**同一
 *          路输出上相邻脉冲上升沿**的时间, 取第 2 个及以后的间隙(首个脉冲由
 *          Pulse_TriggerFireAll 硬件触发, 开销与后续软件重触发不同), 跨 1/2/4/8/16µs
 *          五点对照上表。注意**不要跨通道测量** —— 探头偏斜在 4µs 上量 70ns 时
 *          足以盖过信号。
 */
